...
if(!class_exists('Cosy_WooCommerce')) {

    class Cosy_WooCommerce{
        ...
        public function __construct(){
            ...
            add_filter('cosy/get_site_layout', array( $this, 'set_site_layout') );

            add_filter('cosy/filter/sidebar_primary_name', array( $this, 'set_sidebar_for_shop'), 20 );
            add_filter('cosy/setting/get_setting_by_context', array( $this, 'override_setting_by_context'), 20, 3);
            ...
            add_action('woocommerce_before_subcategory_title', create_function('', ' echo "<span class=\"item--overlay\"></span></div>";'), 11);
            ...
            add_action('cosy/action/shop_loop_item_action', create_function('', ' echo "<div class=\"wrap-addto\">";'), 4 );
            add_action('cosy/action/shop_loop_item_action', 'woocommerce_template_loop_add_to_cart', 12 );
            add_action('cosy/action/shop_loop_item_action', array( $this, 'add_wishlist_btn' ), 14 );
            add_action('cosy/action/shop_loop_item_action', array( $this, 'add_compare_btn' ), 16 );
            add_action('cosy/action/shop_loop_item_action', array( $this, 'add_quick_view_btn' ), 18 );
            add_action('cosy/action/shop_loop_item_action', create_function('', 'echo "</div>";'), 20 );
            ...
            add_action('woocommerce_single_product_summary', array( $this, 'add_stock_into_single' ), 1);
            ...
            add_action('woocommerce_single_product_summary', create_function('', ' echo "<div class=\"clearfix\"></div>";'), 50);
            ...
            if( Cosy()->settings->get('catalog_mode', 'off') == 'on'){
                // In Loop
                remove_action( 'woocommerce_after_shop_loop_item', 'woocommerce_template_loop_add_to_cart', 10 );
        ...
        public function set_sidebar_for_shop( $sidebar ){
            ...
            if( in_array( 'is_woocommerce', $context ) ){
                ...
                if(in_array( 'is_archive', $context ) ){
                    ...
                    $sidebar = Cosy()->settings->get('shop_sidebar', $sidebar);

                    if(Cosy()->settings->get('shop_global_sidebar', false)){
                        /*
                    ...
                    if(in_array( 'is_shop', $context)){
                        if( $single_sidebar = Cosy()->settings->get_post_meta( Cosy()->get_page_id(), 'sidebar') && !empty($single_sidebar) ){
                            $sidebar = $single_sidebar;
                        }
                    }
                    if(in_array( 'is_product_taxonomy', $context)){
                        if( $tax_sidebar = Cosy()->settings->get_term_meta( get_queried_object_id(), 'sidebar') && !empty($tax_sidebar) ){
                            $sidebar = $tax_sidebar;
                        }
                    }
                }
                ...
                elseif(in_array('is_product', $context)){
                    $sidebar = Cosy()->settings->get('products_sidebar', $sidebar);

                    if(Cosy()->settings->get('products_global_sidebar', false)){
                        /*
                         * Return global sidebar if option will be enable
                         * We don't need more checking in context
                         */
                    ...
                    if( $single_sidebar = Cosy()->settings->get_post_meta( get_the_ID(), 'sidebar') && !empty($single_sidebar) ){
                        $sidebar = $single_sidebar;
                    }
                }
            }
        ...
        public function addPostClassToLoop($classes, $class, $post_id){
            ...
            if ( $product ) {
                ...
                $show_image = false;
                if( 'on' == Cosy()->settings->get('woocommerce_enable_crossfade_effect') ){
                    $show_image = true;
                }
                ...
                if($show_image && (($galleries = $product->get_gallery_image_ids()) && !empty($galleries[0]))){
                    ...
                    if(!empty($cosy_loop['loop_layout']) && !empty($cosy_loop['loop_style']) && $cosy_loop['loop_layout'] == 'list' && $cosy_loop['loop_style'] == 'special'){
                ...
                $classes[] = 'prod-rating-' . esc_attr(Cosy()->settings->get('woocommerce_show_rating_on_catalog', 'off'));
            }
        ...
        public function add_script_resize_image_in_loop(){
            global $cosy_loop;
            if(!empty($cosy_loop['image_size'])) {
                Cosy()->images->before_resize();
            }
        }
        ...
        public function modifyProductThumbnailSize($size){
            global $cosy_loop;
            if(!empty($cosy_loop['image_size'])){
                return $cosy_loop['image_size'];
            }
            return $size;
        }

        public function add_second_thumbnail_to_loop(){
            global $cosy_loop, $product;
            $show_image = false;
            if( 'on' == Cosy()->settings->get('woocommerce_enable_crossfade_effect') ){
                $show_image = true;
            }
        ...
        public static function add_multi_thumbnail_to_loop(){
            ...
            if(($galleries = $product->get_gallery_image_ids()) && !empty($galleries)){
                ...
                foreach($galleries as $gallery){
                    $i++;
                    printf('<a href="%s"><span class="thumb-multi-item" style="background-image: url(%s)"></span></a>',
                        get_the_permalink(),
                        wp_get_attachment_image_url($gallery, apply_filters( 'single_product_archive_thumbnail_size', 'shop_catalog' ))
                    );
                    if($i == 2){
                        break;
                    }
        ...
        public function add_quick_view_btn(){
            if( 'on' == Cosy()->settings->get('woocommerce_show_quickview_btn', 'off') ){
                global $product;
                printf(
        ...
        public function add_compare_btn(){
            ...
            if (!empty($yith_woocompare->obj) && Cosy()->settings->get('woocommerce_show_compare_btn', 'off') == 'on') {
        ...
        public function add_wishlist_btn(){
            ...
            if (function_exists('YITH_WCWL') && Cosy()->settings->get('woocommerce_show_wishlist_btn', 'off') == 'on') {
                global $product;
                ...
                if (YITH_WCWL()->is_product_in_wishlist($product->get_id(), $default_wishlist)) {
                    $text = esc_html__('View Wishlist', 'cosy');
                    $class = 'add_wishlist button added';
                    $url = YITH_WCWL()->get_wishlist_url('');
                } else {
                    $text = esc_html__('Add to Wishlist', 'cosy');
        ...
        public function set_cookie_default(){
            if (isset($_GET['per_page']) && $per_page = $_GET['per_page']) {
                add_filter('cosy/filter/product_per_page', array( $this, 'getParameterPerPage'));
            }
        }
        ...
        public function addCountUpTimerToSingleProduct(){
            if(!isset($_GET['product_quickview']) && Cosy()->settings->get('show_product_countdown')){
                global $product;
        ...
        public function checkConditionShowUpsellAndCrosssell(){
            if ( Cosy()->settings->get('related_products', 'off') != 'on' ) {
                remove_action('woocommerce_after_single_product_summary', 'woocommerce_output_related_products', 20);
            }
            if ( Cosy()->settings->get('upsell_products', 'off') != 'on' ) {
                remove_action('woocommerce_after_single_product_summary', 'woocommerce_upsell_display', 15);
            }
        }

        public function addCustomTabs($tabs){
            if ( Cosy()->settings->get('woocommerce_product_enable_customtab', 'off') == 'on' ) {
                $tabs['custom_tab'] = array(
                    'title' => Cosy()->settings->get('woocommerce_product_customtab_title', ''),
                    'priority' => 50,
        ...
        public function getCustomTabsContent(){
            echo Cosy_Helper::remove_js_autop( Cosy()->settings->get('woocommerce_product_customtab_content', ''));
        }
            ...
            if (isset($_GET['product_quickview']) && is_product() && $product->is_type('variable')) {
                ...
                <script type="text/javascript">
                    ...
                    var wc_add_to_cart_variation_params = <?php echo json_encode(array(
                        'i18n_no_matching_variations_text' => esc_attr__( 'Sorry, no products matched your selection. Please choose a different combination.', 'cosy' ),
                        'i18n_make_a_selection_text'       => esc_attr__( 'Select product options before adding this product to your cart.', 'cosy' ),
                        'i18n_unavailable_text'            => esc_attr__( 'Sorry, this product is unavailable. Please choose a different combination.', 'cosy' )
                    )); ?>;
                    /* ]]> */
                </script>
        ...
        public function modify_ajax_cart_fragments( $fragments ){
            ...
            $text = '<span class="la-cart-text">'. esc_html__('%s items', 'cosy') .'</span>';
        ...
        public function addProgressBarToCartPage(){
            ...
            <div class="row section-checkout-step">
                <div class="col-xs-12">
                    <ul>
                        <li class="step-1 first"><span class="step-num cosy-icon-cart-modern-in"></span><span class="step-name"><?php esc_html_e('Your Cart', 'cosy') ?></span>
                        </li><li class="step-2"><span class="step-num cosy-icon-card-edit"></span><span class="step-name"><?php esc_html_e('Check out', 'cosy') ?></span>
                        </li><li class="step-3 last"><span class="step-num cosy-icon-card-edit"></span><span class="step-name"><?php esc_html_e('order complete', 'cosy') ?></span></li>
                    </ul>
                </div>
            </div>
            <?php
        ...
        public function addProgressBarToCheckoutPage(){
            if(is_checkout()){
                add_action('cosy/action/before_render_main_content', array( $this, 'addProgressBarToCartPage' ), 999 );
            }
        }
        ...
        private function override_page_title_bar_setting( $value, $key, $context ){
            ...
            if ( $key == 'page_title_bar_layout') {
                $new_value = Cosy()->settings->$func($current_id, $key);
                if($new_value && $new_value != 'inherit'){
                    return $new_value;
                }
            }

            if( Cosy()->settings->$func($current_id, 'page_title_bar_style') == 'yes' && in_array($key, $arr2)){
                return $value;
            }

            $enable_override = Cosy()->settings->get('woo_override_page_title_bar', 'off');
            if($enable_override == 'on'){
                $new_key = 'woo_' . $key;
                return Cosy()->settings->get($new_key, $value);
            }
            return $value;
        }
    }
