...
class Cosy {
    ...
    private static $original_option_name = 'cosy_options';
    ...
    private function __construct() {
        ...
        wp_cache_add_non_persistent_groups( 'cosy' );
        ...
        $this->settings       = Cosy_Setting::get_instance();
        $this->init           = new Cosy_Init();
        $this->template       = new Cosy_Template();
        $this->blog           = new Cosy_Blog();
        $this->images         = new Cosy_Images();
        $this->layout         = new Cosy_Layout();

        add_action( 'wp', array( $this, 'set_page_id' ) );
        add_action( 'wp', array( $this, 'set_current_context' ) );
        add_action( 'wp', array( $this, 'build_crumbs'));

        new Cosy_Widget_CssClass();

        do_action( 'cosy_core_loaded' );
    }
    ...
    public function build_crumbs(){
        ...
        $this->breadcrumbs = new Cosy_Breadcrumbs( $args );
    }

    public function set_is_updating() {
        if ( ! self::$is_updating && $_GET && isset( $_GET['cosy_update'] ) && '1' == $_GET['cosy_update'] ) {
    ...
    public static function multilingual_options() {
        // Set the self::$lang.
        if ( ! in_array( Cosy_Multilingual::get_active_language(), array( '', 'en', 'all' ) ) ) {
            self::$lang = '_' . Cosy_Multilingual::get_active_language();
        }
