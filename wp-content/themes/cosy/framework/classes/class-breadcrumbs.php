...
class Cosy_Breadcrumbs {
	...
	public function __construct( $args = array() ) {
		...
		$defaults = array(
			...
			'page_title_format' => '<header><div class="page-title h1">%s</div></header>',
			...
			'labels'            => array(),
			'date_labels'       => array(),
			'action'            => 'cosy/action/breadcrumbs/render_html',
			'css_namespace'     => array(),
			'path_type'         => 'full', // minified , full
			'post_taxonomy'     => apply_filters(
				'cosy/filter/breadcrumbs/trail_taxonomies',
				array(
					'post'      => 'category',
				)
			),
		...
		$this->args = apply_filters( 'cosy/filter/breadcrumbs/args', wp_parse_args( $args, $defaults ) );
	...
	public function get_trail() {
		...
		$wrapper_classes = apply_filters( 'cosy/filter/breadcrumbs/wrapper_classes', $wrapper_classes );
	...
	public function get_items() {
		...
		$breadcrumb = apply_filters( 'cosy/filter/breadcrumbs/trail', $breadcrumb, $this->args );
	...
	public function default_labels() {
		...
		if ( $use_custom_front_title ) {
			$default = esc_html__( 'Home', 'cosy' );
			$page_on_front_title = $default;
			$page_on_front_title = $this->prepare_label( $page_on_front_title, $default );
		}

		$labels = array(
			'browse'              => esc_html__( 'Browse:', 'cosy' ),
			'home'                => $page_on_front_title,
			'error_404'           => esc_html__( '404 Not Found', 'cosy' ),
			'archives'            => esc_html__( 'Archives', 'cosy' ),
			'search'              => esc_html__( 'Search results', 'cosy' ),
			'paged'               => esc_html__( 'Page %s', 'cosy' ),
			'archive_minute'      => esc_html__( 'Minute %s', 'cosy' ),
			'archive_week'        => esc_html__( 'Week %s', 'cosy' ),
	...
	public function build_trail() {
		...
		$custom_trail = apply_filters( 'cosy/filter/breadcrumbs/custom_trail', false, $this->args );
		 ...
		 * @var   array
		 */
		$this->items = apply_filters( 'cosy/filter/breadcrumbs/items', $this->items, $this->args );
...
//			$this->_add_item( 'link_format', $label, $url );
	...
	public function prepare_label( $label, $fallback = false ) {
		...
		if ( false !== $fallback ) {
			$fallback = sprintf(
				apply_filters( 'cosy/filter/breadcrumbs/icon_fallback', '<span class="hidden">%s</span>', $fallback ),
				esc_attr( $fallback )
			);
		}
		...
		// Return default icon
		return sprintf(
			apply_filters( 'cosy/filter/breadcrumbs/default_icon_format', '<span class="%1$s">%2$s</span>', $label, $fallback ),
