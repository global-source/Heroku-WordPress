...
class Cosy_Autoload {
    ...
    public function __construct( $args = array() ) {
        ...
        if ( empty( self::$transient_name ) ) {
            self::$transient_name = 'cosy_autoloader_paths_' . md5( __FILE__ );
        }
    ...
    protected function get_path( $class_name ) {
        ...
        $paths = array();
        if ( 0 === stripos( $class_name, 'Cosy' ) ) {
            $path     = wp_normalize_path( Cosy::$template_dir_path . '/framework/classes/' );

            $class_name = str_replace( array( 'Cosy_' ), '', $class_name );
