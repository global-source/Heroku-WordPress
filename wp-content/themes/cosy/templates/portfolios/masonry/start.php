<?php
global $cosy_loop;
$loop_id = isset($cosy_loop['loop_id']) ? $cosy_loop['loop_id'] : uniqid('la-show-portfolios-');
$style = isset($cosy_loop['loop_style']) ? $cosy_loop['loop_style'] : 'default';
$responsive_column = isset($cosy_loop['responsive_column']) ? $cosy_loop['responsive_column'] : array('xlg'=> 1, 'lg'=> 1,'md'=> 1,'sm'=> 1,'xs'=> 1);

$enable_skill_filter = isset($cosy_loop['enable_skill_filter']) ? true : false;
$filter_style = isset($cosy_loop['filter_style']) ? $cosy_loop['filter_style'] : '1';
$filters = isset($cosy_loop['filters']) ? $cosy_loop['filters'] : '';
...
<?php if($enable_skill_filter): ?>
    <div class="la-isotope-filter-container filter-style-<?php echo esc_attr($filter_style);?>" data-isotope_container="#<?php echo esc_html($loop_id) ?> .la-isotope-container">
        <div class="la-toggle-filter"><?php esc_html_e('All', 'cosy'); ?></div><ul><li class="active" data-filter="*"><a href="#"><?php esc_html_e('All', 'cosy'); ?></a></li><?php
            if(!empty($filters)){
